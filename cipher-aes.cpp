...
static inline
void
key_add8to32 (BLOCK const& s, std::uint32_t const *keys, std::uint32_t *out)
{
...
static inline
void
key_add32to8(std::uint32_t const *s, std::uint32_t const *keys, BLOCK& out)
{
...
void
AES::set_key128 (std::array<std::uint8_t,16> const& key)
{
    enum { NK = 4, NR = 10 };
    for (int i = 0; i < NK; ++i) {
        int const j = i * 4;
...
void
AES::set_key192 (std::array<std::uint8_t,24> const& key)
{
    enum { NK = 6, NR = 12 };
    for (int i = 0; i < NK; ++i) {
        int const j = i * 4;
...
void
AES::set_key256 (std::array<std::uint8_t,32> const& key)
{
    enum { NK = 8, NR = 14 };
    for (int i = 0; i < NK; ++i) {
        int const j = i * 4;
...
void
AES::schedule_keys (int const nk, int const nr)
{
